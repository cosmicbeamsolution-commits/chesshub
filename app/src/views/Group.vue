      ...
      <div class="columns">
        ...
        <div class="column">
          ...
          <div v-show="tab === 'playing'">
            <div class="group min-20" v-if="onlineGames.length" :class="{ 'no-players': players.length < 2 }">
...
export default {
  ...
  sockets: {
    ...
    group_join (data) {
      setTimeout(() => {
        this.chatLines.push({
          text: `<span class="mdi mdi-arrow-right has-text-success"></span> <span>${data.flag}</span> ${data.code}`,
          ts: moment().fromNow(true),
          sender: 'bot',
          owned: false
        })
    ...
    group_leave (data) {
      this.chatLines.push({
        text: `<span class="mdi mdi-arrow-left has-text-success"></span> <span>${data.flag}</span> ${data.code}`,
        ts: moment().fromNow(true),
        sender: 'bot',
        owned: false
      })
          ...
          swal({
            ...
            .then(accept => {
              if (accept) {
                ...
                this.$socket.emit('action', { ...data, action: 'reject' })
      ...
      <div class="field">
        <div class="control has-text-centered column">
          <div class="buttons levels has-addons groupprivacy">
            ...
            <button class="button is-toggle">${this.$root.t('private')}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="field">
        <div class="control has-text-centered column">
          <div class="buttons levels has-addons groupgames">
            ...
            <button class="button is-toggle">16</button>
          </div>
        </div>
      </div>
      <label>
    ...
    <div class="column">
      ...
      <div class="control has-text-centered column">
        <div class="buttons levels has-addons groupgames" title="Nro. partidas de este match">
          <button class="button is-toggle has-background-success" title="Match a 1 partida">1</button>
          ...
          <button class="button is-toggle" title="Match a 16 partidas">16</button>
        </div>
      </div>
      ...
      <!--div class="control" title="Selecciona el color de tus piezas">
        <div class="buttons levels has-addons playercolor">
          <button class="button is-toggle is-large has-background-success is-white-pieces" title="Jugar con blancas">
          </button>
          <button class="button is-toggle is-large is-random-pieces" title="Que vaya a sorteo">
          </button>
          <button class="button is-toggle is-large is-black-pieces" title="Jugar con negras">
          </button>
        </div>
      </div-->
    </div>
    ...
    <div class="column">
      ...
      <div class="control has-text-centered column">
        <div class="buttons levels has-addons gameclock" title="Establece la duración de la partida en minutos">
          <button class="button is-toggle has-background-success" title="Agregar 3 minutos a la partida">3'</button>
          ...
          <button class="button is-toggle" title="Agregar 30 minutos a la partida">30'</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="column">
      ...
      <div class="control has-text-centered column">
        <div class="buttons levels has-addons gamecompensation" title="Agregar compensación por movimiento">
          <button class="button is-toggle" title="Jugar sin compensación por movimiento">+0</button>
          ...
          <button class="button is-toggle" title="Agregar 3 segundos de compensación por cada movimiento">+3</button>
        </div>
      </div>
    </div>
  </div>
