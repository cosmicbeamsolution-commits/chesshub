...
module.exports = {
  login: (req, res) => {
    var email = req.body.email_or_username.toLowerCase()
    ...
    req.app.db.collection('accounts').findOne({
      ...
      if (!passwordIsValid) return res.status(404).send(`Your password is wrong`)
  ...
  create: (req, res) => {
    ...
    return req.app.db.collection('accounts').findOneAndUpdate(
      { '_id': new ObjectId(id) },
      { "$set": req.body },
      {
        upsert: true,
